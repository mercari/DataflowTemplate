...
public class Count extends WindowProcessor {

    private final Map<String, Integer> bufferSizes;

    public Count(final String name, final String condition, final Boolean ignore, final JsonObject params) {

        super(name, condition, ignore, params);

        if(ranges.size() == 0) {
            throw new IllegalArgumentException("Count step: " + name + " requires ranges over size zero");
        }

        this.bufferSizes = super.getBufferSizes();
    }

    public static Count of(final String name, final String condition, final Boolean ignore, final JsonObject params) {
        return new Count(name, condition, ignore, params);
    }

...
public class Sum extends WindowProcessor {

    private final Map<String, Integer> bufferSizes;

    public Sum(final String name, final String condition, final Boolean ignore, final JsonObject params) {

        super(name, condition, ignore, params);

        if(ranges.size() == 0) {
            throw new IllegalArgumentException("Sum step: " + name + " requires ranges over size zero");
        }

        this.bufferSizes = super.getBufferSizes();
    }

    public static Sum of(final String name, final String condition, final Boolean ignore, final JsonObject params) {
        return new Sum(name, condition, ignore, params);
    }
    ...
    public Map<String, Schema.FieldType> getBufferTypes(Map<String, Schema.FieldType> inputTypes) {
        ...
        if(this.fields.size() > 0) {
            for(final String field : fields) {
                ...
                if(fieldType == null) {
                    throw new IllegalArgumentException("Sum step: " + name + " input field: " + field + " not found");
                }
            ...
            for(final String variable : bufferSizes.keySet()) {
                ...
                if(fieldType == null) {
                    throw new IllegalArgumentException("Sum step: " + name + " input variable: " + variable + " not found");
                }
                ...
                for(int e=0; e<expressions.size(); e++) {
                    ...
                    for(int r=range.getKey(); r<range.getValue(); r++) {
                        ...
                        try {
                            ...
                            LOG.debug("Sum step: " + name + " failed evaluate expression: " + expressions.get(e) + " for variables: " + values + " cause: " + ee);
                        }
                    }
